...
function requireParse$1() {
  ...
  const parse = (input, options) => {
    ...
    if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{}"])/.test(input)) {
      ...
      state2.output = utils2.wrapOutput(output, state2, options);
      return state2;
    }
    while (!eos()) {
      value = advance();
...
function requireReaddirp() {
  ...
  const { promisify: promisify2 } = require$$2;
  const picomatch2 = requirePicomatch();
  const readdir = promisify2(fs2.readdir);
  const stat = promisify2(fs2.stat);
  const lstat = promisify2(fs2.lstat);
  const realpath = promisify2(fs2.realpath);
  const BANG = "!";
...
function requireNodefsHandler() {
  ...
  const open = promisify2(fs2.open);
  const stat = promisify2(fs2.stat);
  const lstat = promisify2(fs2.lstat);
  const close = promisify2(fs2.close);
  const fsrealpath = promisify2(fs2.realpath);
...
var fsevents$2 = {};
const fsevents = require2("./chunks/fsevents-mvLcArqM.node");
const fsevents$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: fsevents
}, Symbol.toStringTag, { value: "Module" }));
const require$$0 = /* @__PURE__ */ getAugmentedNamespace(fsevents$1);
var hasRequiredFsevents;
function requireFsevents() {
  if (hasRequiredFsevents) return fsevents$2;
  ...
  fsevents$2.watch = watch;
  fsevents$2.getInfo = getInfo;
  fsevents$2.constants = events;
  return fsevents$2;
}
...
function requireFseventsHandler() {
  ...
  const createFSEventsInstance = (path2, callback) => {
    const stop = fsevents2.watch(path2, callback);
    return { stop };
  };
    ...
    if (cont || watchedParent) {
      ...
      cont = {
        ...
        watcher: createFSEventsInstance(watchPath, (fullPath, flags) => {
          ...
          const info = fsevents2.getInfo(fullPath, flags);
  ...
  const canUse = () => fsevents2 && FSEventsWatchers.size < 128;
...
function requireChokidar() {
  ...
  class FSWatcher extends EventEmitter2 {
